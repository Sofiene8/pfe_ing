<?php
/* Smarty version 4.3.0, created on 2026-03-04 11:43:28
  from 'template_jobsquare_user:listing_item.tpl' */

/* @var Smarty_Internal_Template $_smarty_tpl */
if ($_smarty_tpl->_decodeProperties($_smarty_tpl, array (
  'version' => '4.3.0',
  'unifunc' => 'content_69a81ae027b9a2_26402696',
  'has_nocache_code' => false,
  'file_dependency' => 
  array (
    'aa63f2abf70bbb390ba75f165449c568f90359ba' => 
    array (
      0 => 'template_jobsquare_user:listing_item.tpl',
      1 => 1772573832,
      2 => 'template_jobsquare_user',
    ),
  ),
  'includes' => 
  array (
  ),
),false)) {
function content_69a81ae027b9a2_26402696 (Smarty_Internal_Template $_smarty_tpl) {
$_smarty_tpl->_checkPlugins(array(0=>array('file'=>'C:\\xampp\\htdocs\\JobsquareMa\\system\\ext\\Smarty\\libs\\plugins\\modifier.truncate.php','function'=>'smarty_modifier_truncate',),));
?>
<div class="sj-job-card<?php if ($_smarty_tpl->tpl_vars['listing']->value['featured']) {?> sj-featured<?php }?>" id="<?php echo $_smarty_tpl->tpl_vars['listing']->value['id'];?>
">
    <?php if ($_smarty_tpl->tpl_vars['listing']->value['featured']) {?>
        <span class="sj-badge-new">NEW</span>
    <?php }?>

    <?php if ($_smarty_tpl->tpl_vars['listing']->value['featured'] == 1 || $_smarty_tpl->tpl_vars['listing']->value['user']['featured'] == 1) {?>
        <div class="sj-card-logo">
            <?php if ($_smarty_tpl->tpl_vars['listing']->value['user']['Logo']['file_url']) {?>
                <a href="<?php echo $_smarty_tpl->tpl_vars['GLOBALS']->value['site_url'];
echo call_user_func_array($_smarty_tpl->registered_plugins[ 'modifier' ][ 'listing_url' ][ 0 ], array( $_smarty_tpl->tpl_vars['listing']->value ));?>
?backPage=<?php echo $_smarty_tpl->tpl_vars['pageForBackButton']->value;?>
&searchID=<?php echo $_smarty_tpl->tpl_vars['searchId']->value;?>
">
                    <img src="<?php echo $_smarty_tpl->tpl_vars['listing']->value['user']['Logo']['file_url'];?>
" alt="<?php echo htmlspecialchars((string)$_smarty_tpl->tpl_vars['listing']->value['user']['CompanyName'], ENT_QUOTES, 'UTF-8', true);?>
">
                </a>
            <?php } else { ?>
                <?php echo mb_strtoupper((string) smarty_modifier_truncate(htmlspecialchars((string)$_smarty_tpl->tpl_vars['listing']->value['user']['CompanyName'], ENT_QUOTES, 'UTF-8', true),4,'',true) ?? '', 'UTF-8');?>

            <?php }?>
        </div>
    <?php }?>

    <div class="sj-card-content">
        <div class="sj-card-title">
            <a href="<?php echo $_smarty_tpl->tpl_vars['GLOBALS']->value['site_url'];
echo call_user_func_array($_smarty_tpl->registered_plugins[ 'modifier' ][ 'listing_url' ][ 0 ], array( $_smarty_tpl->tpl_vars['listing']->value ));?>
?backPage=<?php echo $_smarty_tpl->tpl_vars['pageForBackButton']->value;?>
&searchID=<?php echo $_smarty_tpl->tpl_vars['searchId']->value;?>
">
                <?php echo htmlspecialchars((string)$_smarty_tpl->tpl_vars['listing']->value['Title'], ENT_QUOTES, 'UTF-8', true);?>

            </a>
        </div>

        <div class="sj-card-company">
            <a href="<?php echo $_smarty_tpl->tpl_vars['GLOBALS']->value['site_url'];
echo call_user_func_array($_smarty_tpl->registered_plugins[ 'modifier' ][ 'listing_url' ][ 0 ], array( $_smarty_tpl->tpl_vars['listing']->value ));?>
?backPage=<?php echo $_smarty_tpl->tpl_vars['pageForBackButton']->value;?>
&searchID=<?php echo $_smarty_tpl->tpl_vars['searchId']->value;?>
">
                <?php echo htmlspecialchars((string)$_smarty_tpl->tpl_vars['listing']->value['user']['CompanyName'], ENT_QUOTES, 'UTF-8', true);?>

            </a>
            <?php if (call_user_func_array($_smarty_tpl->registered_plugins[ 'modifier' ][ 'location' ][ 0 ], array( $_smarty_tpl->tpl_vars['listing']->value ))) {?>
                <span class="sj-sep">&middot;</span>
                <?php echo call_user_func_array($_smarty_tpl->registered_plugins[ 'modifier' ][ 'location' ][ 0 ], array( $_smarty_tpl->tpl_vars['listing']->value ));?>

            <?php }?>
        </div>

        <div class="sj-card-desc">
            <?php echo smarty_modifier_truncate(preg_replace('!<[^>]*?>!', ' ', $_smarty_tpl->tpl_vars['listing']->value['JobDescription'] ?: ''),250,"...");?>

        </div>

        <div class="sj-card-tags">
            <?php if (call_user_func_array($_smarty_tpl->registered_plugins[ 'modifier' ][ 'location' ][ 0 ], array( $_smarty_tpl->tpl_vars['listing']->value ))) {?>
                <span class="sj-card-tag sj-loc"><?php echo call_user_func_array($_smarty_tpl->registered_plugins[ 'modifier' ][ 'location' ][ 0 ], array( $_smarty_tpl->tpl_vars['listing']->value ));?>
</span>
            <?php }?>
            <?php
$_from = $_smarty_tpl->smarty->ext->_foreach->init($_smarty_tpl, $_smarty_tpl->tpl_vars['listing']->value['EmploymentType'], 'list_value', false, NULL, 'multifor', array (
  'first' => true,
  'index' => true,
));
$_smarty_tpl->tpl_vars['list_value']->do_else = true;
if ($_from !== null) foreach ($_from as $_smarty_tpl->tpl_vars['list_value']->value) {
$_smarty_tpl->tpl_vars['list_value']->do_else = false;
$_smarty_tpl->tpl_vars['__smarty_foreach_multifor']->value['index']++;
$_smarty_tpl->tpl_vars['__smarty_foreach_multifor']->value['first'] = !$_smarty_tpl->tpl_vars['__smarty_foreach_multifor']->value['index'];
?>
                <?php if ((isset($_smarty_tpl->tpl_vars['__smarty_foreach_multifor']->value['first']) ? $_smarty_tpl->tpl_vars['__smarty_foreach_multifor']->value['first'] : null) && $_smarty_tpl->tpl_vars['list_value']->value) {?>
                    <span class="sj-card-tag sj-type"><?php $_block_plugin45 = isset($_smarty_tpl->smarty->registered_plugins['block']['tr'][0][0]) ? $_smarty_tpl->smarty->registered_plugins['block']['tr'][0][0] : null;
if (!is_callable(array($_block_plugin45, 'translate'))) {
throw new SmartyException('block tag \'tr\' not callable or registered');
}
$_smarty_tpl->smarty->_cache['_tag_stack'][] = array('tr', array());
$_block_repeat=true;
echo $_block_plugin45->translate(array(), null, $_smarty_tpl, $_block_repeat);
while ($_block_repeat) {
ob_start();
echo $_smarty_tpl->tpl_vars['list_value']->value;
$_block_repeat=false;
echo $_block_plugin45->translate(array(), ob_get_clean(), $_smarty_tpl, $_block_repeat);
}
array_pop($_smarty_tpl->smarty->_cache['_tag_stack']);?></span>
                <?php }?>
            <?php
}
$_smarty_tpl->smarty->ext->_foreach->restore($_smarty_tpl, 1);?>
        </div>
    </div>

    <div class="sj-card-right">
        <span class="sj-card-date">
            <?php if (!$_smarty_tpl->tpl_vars['listing']->value['featured']) {
echo call_user_func_array($_smarty_tpl->registered_plugins[ 'modifier' ][ 'date' ][ 0 ], array( $_smarty_tpl->tpl_vars['listing']->value['activation_date'] ));
}?>
        </span>
        <a href="<?php echo $_smarty_tpl->tpl_vars['GLOBALS']->value['site_url'];
echo call_user_func_array($_smarty_tpl->registered_plugins[ 'modifier' ][ 'listing_url' ][ 0 ], array( $_smarty_tpl->tpl_vars['listing']->value ));?>
?backPage=<?php echo $_smarty_tpl->tpl_vars['pageForBackButton']->value;?>
&searchID=<?php echo $_smarty_tpl->tpl_vars['searchId']->value;?>
" class="sj-card-btn">
            Voir Plus
        </a>
    </div>
</div><?php }
}
