...
class SJB_TemplateProcessor extends Smarty
{
	...
	function __construct($templatesupplier)
	{
		...
		$this->compile_check = (SJB_System::getSystemSettings('HTTPHOST') === 'localhost');
        ...
        $this->registerPlugin('modifier', 'blog_url', [&$this, 'blog_url']);
		...
		$this->registerPlugin('modifier', 'status', array(&$this, 'status'));
...
//	public static function date($date, $format = null, $withTime = false)
//	{
...
//		}
//		return SJB_DateFormatter::localizeFormat($date, false);
//	}

    public static function date($date, $format = null, $withTime = false)
    {
        $f = new SJB_DateFormatter();
        return $f->getOutput($date, $format, $withTime);
    }
	 ...
	 */
	public static function location($listingOrUser)
	{
		if ($listingOrUser) {
			...
			if (!empty($listingOrUser['Location']['City'])) {
				return $listingOrUser['Location']['City'];
			}
		}
		return '';
	}
	...
	public static function listing_url($listing)
	{
		...
		if ($listing && is_array($listing)) {
            ...
            } elseif ($listing['type']['id'] == 'Training') {
                $url = '/training/' . $listing['id'] . '/' . self::pretty_url($listing['Title']) . '/';
			} else {
    ...
    public static function blog_url($post)
    {
        if (!trim($post['url'])) {
            $post['url'] = sprintf('/%s/%s/', $post['sid'], static::pretty_url($post['title']));
        }
	...
	public static function _tpl_javascript($params, $content)
	{
		if (empty($content)) {
			return false;
		}
	...
	public function fetch($template = null, $cache_id = null, $compile_id = null, $parent = null, $display = false, $merge_tpl_vars = true, $no_output_filter = false)
	{
		try {
		    ...
		    SJB_Error::getInstance()->addError($ex->getMessage(), ['exception' => $ex]);
	...
	{
		parent::display($this->templateSupplier->getTplName($template), $cache_id, $this->module_name, $parent);
	}
	...
	public static function deleteCache()
	{
		...
		if (empty($cacheDir)) {
			return 'Unable to get cache dir';
		}
		if (file_exists($cacheDir)) {
			$iterator = new RecursiveDirectoryIterator($cacheDir, FilesystemIterator::SKIP_DOTS);
			$iterator = new RecursiveIteratorIterator($iterator);
			foreach ($iterator as $file) {
				if (preg_match('/.php$/iu', $file->getPathName())) {
					if (!@unlink($file->getPathName()) ) {
						return 'Unable to delete file ' . $file->getPathName();
					}
				}
			}
		}
