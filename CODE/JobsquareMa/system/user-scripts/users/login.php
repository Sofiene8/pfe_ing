...
class SJB_Users_Login extends SJB_Function
{
	public function execute()
	{
		...
		if (SJB_Authorization::isUserLoggedIn() && !isset($_REQUEST['as_user'])) {
			...
			if (SJB_Request::getVar('action', false) == 'login' || SJB_Request::getVar('network')) {
				...
				if (isset($_REQUEST['as_user'])) {
					...
					$login_as_user = SJB_Admin::admin_authed();
				...
				if (SJB_UserManager::getCurrentUserSID()) {
				    ...
				    if (SJB_Request::getVar('network')) {
				        ...
				        if ($user) {
                            $logged_in = SJB_Authorization::login($user['username'], $user['password'], true, $errors, true);
                        } else {
				            $userGroup = '';
				            if (SJB_Request::getVar('user_group_id')) {
                                $userGroup = '&user_group_id=' . SJB_Request::getVar('user_group_id');
                            }
				            SJB_H::redirect(SJB_H::getCustomDomainUrl() . '/registration/?network=' . SJB_Request::getVar('network') . $userGroup);
                        }
                    } else {
			...
			$tp->assign('skip_registration_return', SJB_Request::getVar('skip_registration_return', false));
