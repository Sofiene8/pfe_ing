...
class SJB_Classifieds_FeaturedListingsSponsorise extends SJB_Function
{
    public function execute()
    {
       
	   $template = SJB_Request::getVar('template', 'featured_listings_sponsorise.tpl');
			   ...
			   $listing = SJB_ListingManager::getObjectBySID($listingID);
			    ...
			    Foreach ($listing_structure['JobCategory'] as $key => $JobCategory)
				{
			        $searches['data']['JobCategory']['equal'] = $key;
						$category=	'Offres d\'emploi similaires sponsoriseés ';
				  break;
				}		   
