...
private function handleExportPdf($currentUser, $appJobId, $filters)
{
    ...
    // Debug: Check how many applications after slice
    // var_dump('Applications to process:', count($applications));
    
    if (empty($applications)) {
        // Return JSON error instead of exiting
        echo json_encode([
            'success' => false,
            'message' => 'Aucun CV trouvé à exporter.'
        ]);
        return false;
    }
    ...
    $zipFilename = 'resumes_' . ($downloadAll ? 'all_' : 'page_' . ($_GET['page'] ?? 1) . '_') . $dateSuffix . '.zip';
    $tempZipPath = tempnam(sys_get_temp_dir(), 'zip_');
    ...
    foreach ($applications as $index => $app) {
        ...
        // Try to get listing date - check multiple possible fields
        $listingDate = null;
        ...
        if ($listingDate) {
            ...
            $month = date('m', $listingDate);
            $day = date('d', $listingDate);
            $filePath = $uploadDir . '/applications/' . $year . '/' . $month . '/' . $day . '/' . $listingId . '/' . $fileInfo['saved_file_name'];
        } else {
            // Fallback path without date structure
            $filePath = $uploadDir . '/applications/' . $listingId . '/' . $fileInfo['saved_file_name'];
        }
        ...
        // Remove any dangerous characters and ensure unique filename
        $safeName = $username . '_' . ($index + 1) . '_' . $originalName;
        $safeName = preg_replace('/[^a-zA-Z0-9._-]/', '_', $safeName);

        if (file_exists($filePath) && is_readable($filePath)) {
            // Use addFromString for better reliability
            $fileContent = file_get_contents($filePath);
            if ($fileContent !== false) {
                if ($zip->addFromString($safeName, $fileContent)) {
                    $filesAdded = true;
                    $successfulFiles++;
                } else {
                    $missingFiles++;
            ...
            $alternativePaths = [
                $uploadDir . '/applications/' . $fileInfo['saved_file_name'],
                $uploadDir . '/' . $fileInfo['saved_file_name'],
                dirname($uploadDir) . '/applications/' . $fileInfo['saved_file_name']
            ];
            
            $found = false;
            foreach ($alternativePaths as $altPath) {
                if (file_exists($altPath) && is_readable($altPath)) {
                    $fileContent = file_get_contents($altPath);
                    if ($fileContent !== false && $zip->addFromString($safeName, $fileContent)) {
                        $filesAdded = true;
                        $successfulFiles++;
                        $found = true;
                        break;
    ...
    if (!$filesAdded) {
        ...
        echo json_encode([
            'success' => false,
            'message' => 'Aucun fichier CV trouvé à exporter. ' . $missingFiles . ' fichiers étaient manquants.'
        ]);
        return false;
    }
    ...
    if ($isAjax) {
        // For AJAX, return file URL
        $downloadUrl = SJB_System::getSystemSettings('SITE_URL') . '/temp/' . basename($tempZipPath);
        
        // Move file to accessible location
        $publicTempPath = SJB_System::getSystemSettings('PUBLIC_FILES_DIRECTORY') . '/temp/' . basename($tempZipPath);
        rename($tempZipPath, $publicTempPath);
