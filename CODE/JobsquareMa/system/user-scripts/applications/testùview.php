...
private function handleExportPdf($currentUser, $appJobId, $filters)
{
    ...
    // If some files were missing but some were added, we still download the ZIP
    if ($missingFiles > 0) {
        // You can add a message here if you want to notify about missing files
        // For example, you could store this message in session to display later
        SJB_Session::setValue('export_message', 'Successfully exported ' . ($filesAdded) . ' files. ' . $missingFiles . ' files were missing and skipped.');
    }
