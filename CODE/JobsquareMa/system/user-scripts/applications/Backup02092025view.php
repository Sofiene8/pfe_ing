...
private function getFilteredApplications($currentUser, $appJobId, $filters, $page = 1, $perPage = 20, $getAll = false)
{
    if ($getAll) {
    ...
    private function handleExportCsv($currentUser, $appJobId, $filters)
{
    ...
    // If no applications found, return error
    ...
    // Add BOM for UTF-8 to handle special characters correctly
    ...
    foreach ($applications as $app) {
        $row = [
            ...
            $app['username'] ?? 'N/A',
            $app['email'] ?? 'N/A',
            $app['phone'] ?? 'N/A', // Adjust these field names based on your actual data
            $app['phone2'] ?? 'N/A',
            $app['experience'] ?? 'N/A',
            ...
            $app['website'] ?? 'N/A',
            $app['status'] ?? 'N/A',
            $app['file'] ?? 'N/A' // or $app['resume'] depending on your field
        ];
