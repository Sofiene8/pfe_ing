...
class SJB_Miscellaneous_SitemapGenerator extends SJB_Function
{
 private $lang;
    public function execute()
    {
        ...
        $baseUrl = preg_replace('|http:|u', 'https:', SJB_H::getCustomDomainUrl());

        $handle = fopen(SJB_BASE_DIR . "sitemap.xml", "w");
		
		if ($handle === false) {
		   echo "opening '$handle' failed";
		   exit;
		}
        ...
        fwrite($handle, $this->makeMapItem($baseUrl . '/'));

        // pages
        foreach ($list_of_pages as $page) {
            fwrite($handle, $this->makeMapItem($baseUrl . $page['uri']));
        }
        ...
        foreach ($found_listings_sids as $sid) {
           ...
           fwrite($handle, $this->makeMapItem($baseUrl . SJB_TemplateProcessor::listing_url($listing_info)));
        }
        ...
        foreach ($categoriesInfo['list_values'] as $category) {
            fwrite($handle, $this->makeMapItem($baseUrl . '/categories/' . $category['id'] . '/' . SJB_TemplateProcessor::pretty_url($category['caption']) . '-jobs/'));
        }

        // search results
        fwrite($handle, $this->makeMapItem($baseUrl . '/jobs/'));
        if (SJB_Settings::getValue('public_resume_access')) {
            fwrite($handle, $this->makeMapItem($baseUrl . '/resumes/'));
        }

        // companies
        fwrite($handle, $this->makeMapItem($baseUrl . '/companies/'));
        ...
        foreach ($employers as $employer) {
            ...
            fwrite($handle, $this->makeMapItem($baseUrl . "/company/{$employer['sid']}/" . SJB_TemplateProcessor::pretty_url($employer['CompanyName']) . '/'));
        }
    ...
    private function searchListings($requested_data, $listing_type_id)
    {
		...
		if($listing_type_sid==6)
		{ 
		$date=date('Y-m-d', strtotime(' -6month'));
		$dateArr = explode('-', $date);
		$i18n = SJB_I18N::getInstance();
        $this->lang = $i18n->getLanguageData($i18n->getCurrentLanguage());
		$last_active_date = strftime($this->lang['date_format'], mktime(0, 0, 0, $dateArr[1], $dateArr[2], $dateArr[0]));
        $requested_data['activation_date']['not_less'] = $last_active_date;
