...
class SJB_Applications
{
    ...
    public static function create($listing_id, $jobseeker_id, $resume, $comments, $file, $file_sid, $mimeType, $post = false)
    {
        ...
        $res = SJB_DB::query("insert into applications(`listing_id`, `jobseeker_id`, `comments`, `date`, `resume`, `file`, `username`, `email`, `file_id`, `order`, `status`) values(?s, ?s, ?s, ?s, ?n, ?s, ?s, ?s, ?s, ?s, ?s)", $listing_id, $jobseeker_id ? $jobseeker_id : 0, $comments, SJB_DateType::mysqlNow(), $resume, $file, $jobSeekerName, $jobSeekerEmail, $file_id, time()*-1, $status);
        return !empty($res);
    }
