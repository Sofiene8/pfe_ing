...
class SJB_RefineSearch
{
	...
	public static function countListingsByFieldName($fieldName, $fieldID, $stp, $userField)
	{
		...
		$refineSearchResumeLimit = SJB_Settings::getSettingByName('refine_search_items_resume_limit');
		$limit = $refineSearchLimit ? ' LIMIT 0, ' . $refineSearchLimit : '';
		$limit_resume = $refineSearchResumeLimit ? ' LIMIT 0, ' . $refineSearchResumeLimit : '';
		...
		if ($stp->listing_type_id == 'Resume') {
			$requestedCriteria['access_type'] = array(
...
//                    $query = "SELECT `{$fieldName}` as caption, count(`{$fieldName}`) as count FROM `listings` {$whereStatement} AND `{$fieldName}` != '' GROUP BY `{$fieldName}` ORDER BY count DESC";
                    $query = "SELECT `{$fieldName}` as caption FROM `listings` {$whereStatement}  GROUP BY `{$fieldName}` ";
				...
				if ($userField == 1) {
					$listItem = new SJB_UserProfileFieldListItemManager();
				}
				else{
				...
				$result = $newResult;
				if($stp->listing_type_sid==7)
				{
				if (count($result) > $refineSearchResumeLimit) {
					$result = array_slice($result, 0, $refineSearchResumeLimit);
				}
				}
				else{
			...
			default:
				if ($userField == 1) {
					...
					if($stp->listing_type_sid==7)
                        $query = "SELECT `value`, count(`value`) as count FROM `listings_properties` WHERE `id`='{$fieldName}' AND `value` != '' AND `object_sid` in ({$objectSids}) GROUP BY `value` ORDER BY count DESC {$limit_resume}";
						else
					...
					/*if($fieldName=='Location_City')
					{
					$query ="SELECT cities.sid as sid, cities.name as value FROM cities";
					}
					else
					{*/
//						$query = "SELECT `{$fieldName}` as value, count(`{$fieldName}`) as count FROM `listings` {$whereStatement} AND `{$fieldName}` != '' GROUP BY `{$fieldName}` ORDER BY count DESC {$limit}";
                        //$query = "SELECT `{$fieldName}` as value FROM `listings` {$whereStatement} AND `{$fieldName}` != '' GROUP BY `{$fieldName}` {$limit}";
					if($fieldName=='Location_City' )
					{
					if($stp->listing_type_sid==7)
					$query = "SELECT distinct `{$fieldName}` as value FROM `listings` {$whereStatement} AND `{$fieldName}` != '' {$limit_resume}";
					else
					$query = "SELECT distinct `{$fieldName}` as value FROM `listings` {$whereStatement} AND `{$fieldName}` != '' {$limit}";
					}
					else{
					if($stp->listing_type_sid==7)
					$query = "SELECT `{$fieldName}` as value FROM `listings` {$whereStatement} AND `{$fieldName}` != '' GROUP BY `{$fieldName}` {$limit_resume}";
					else	
					$query = "SELECT `{$fieldName}` as value FROM `listings` {$whereStatement} AND `{$fieldName}` != '' GROUP BY `{$fieldName}` {$limit}";
					}	
	...
	//var_dump($criteria);
		...
		foreach ($criteria as $fieldName => $field) {
			...
			if (!in_array($fieldName, array('listing_type', 'active', 'username', 'status', 'CompanyName', 'keywords', 'PostedWithin'))) {
				...
				$result = array();
				$userFieldSID = SJB_DB::queryValue("SELECT `sid` FROM `user_profile_fields` WHERE `id` = '".$fieldName."'");
				if ($userFieldSID) {
				...
				//$fieldInfo = SJB_UserProfileFieldDBManager::getUserProfileFieldInfoBySID($fieldName);
						...
						if ($userFieldSID)
							...
							if (is_array($fieldValue)) {
								foreach ($fieldValue as $value) {
									if ($value !== '')  {
										if ($fieldInfo['type'] == 'multilist' || $fieldInfo['type'] == 'list') {
	...
	$ids=array();
		...
		foreach ($keys as $key) {
			...
			krsort($ids);
			foreach ($ids as $id) {
			    $catArrayTmp[] = array('caption' => $id, 'count' => 0);
			}
			$catArray=array();
			$catArray=$catArrayTmp;
	...
	{
		$isMultiSelectFieldType   = in_array($refineField['type'], array('multilist'));
	...
	public static function markToShowOrNot(&$foundListingsByFieldName, &$refineField, $criteria)
	{
		foreach ($foundListingsByFieldName as $key => &$elem) {
			if (in_array($refineField['type'], array('multilist', 'list'))) {
