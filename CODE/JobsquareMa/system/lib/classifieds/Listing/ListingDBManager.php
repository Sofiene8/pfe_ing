...
class SJB_ListingDBManager extends SJB_ObjectDBManager
{
    ...
    public static function saveListing($listing, $listingSidsForCopy = [])
    {
		...
		/*	if($listing->getAddDate()=="0000-00-00" || $listing->getAddDate()=="-0001-11-30")
			{
			SJB_DB::query('UPDATE `?w` SET ' .
                '`date_add` = "' . SJB_DateType::mysqlNow() . '"'   . ' WHERE `sid` = ?n',
                'listings', $listing->getSID());
				*/
				SJB_DB::query('UPDATE `?w` SET ' .
                '`date_add` = '.($listing->getAddDate() == null || $listing->getAddDate()=="-0001-11-30" ? '"' . SJB_DateType::mysqlNow() . '"' : "'{$listing->getAddDate()}'")   . ' WHERE `sid` = ?n',
                'listings', $listing->getSID());
			...
			/*}
				...
				SJB_DB::query('UPDATE `?w` SET ' .
                '`update_date` = "' . SJB_DateType::mysqlNow() . '"'   . ' WHERE `sid` = ?n',
                'listings', $listing->getSID());
		//	}
			if($listing_type_sid==7)
            return SJB_DB::query('UPDATE `?w` SET `listing_type_sid` = ?n, `user_sid` = ?n, ' .
                '`activation_date` = ' . ($listing->getActivationDate() == null ? '"' . SJB_DateType::mysqlNow() . '"' : "'{$listing->getActivationDate()}'")  . ' WHERE `sid` = ?n',
                'listings', $listing_type_sid, $listing->getUserSID(), $listing->getSID());
				else
