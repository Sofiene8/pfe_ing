...
class SJB_ListingManager extends SJB_ObjectManager
{
 ...
 public static function boostListingsAfterPaid($userSID, $productSID, $contractID, $listingNumber, $listings = [])
    {
        $serializedProductSID = SJB_ProductsManager::generateQueryBySID($productSID);
	$listing_sid=$listings[0];
         /*$listingsSIDsToProceed = SJB_DB::query("SELECT `sid` FROM `listings` WHERE `checkouted` = 0 AND `contract_id` = 0 AND `user_sid` = ?n AND `product_info` REGEXP '({$serializedProductSID})' ORDER BY `sid` DESC", $userSID);
       foreach ($listingsSIDsToProceed as $sid) {
            if (array_search($sid['sid'], $listings) === false) {
                $listings[] = $sid['sid'];
...
public static function propertyIsUser($property_name)
    {
        if (empty(self::$userProperties)) {
            self::$userProperties = SJB_DB::query("SHOW COLUMNS FROM `users`");
        }
    ...
    public static function canCurrentUserAddListing($listingTypeId)
    {
        ...
        if ($productsInfo) {
            return array_reverse($productsInfo);
        }
        return false;
    }
    ...
    public static function getListingPercentage($listingSID) {
        $listingInfoForPercentage = SJB_ListingManager::getListingInfoBySID($listingSID);
        $percentage = 30;

        if (!empty($listingInfoForPercentage['Photo'])) {
            $percentage += 10;
        }
        if (!empty($listingInfoForPercentage['Education']['ED_DegreeSpecialty'][1]) || !empty($listingInfoForPercentage['Education']['ED_UniversityInstitution'][1])) {
            $percentage += 30;
        }
        if (!empty($listingInfoForPercentage['WorkExperience']['WE_JobTitle'][1])) {
            $percentage += 30;
        }
		else if($listingInfoForPercentage['id_Resume_careerlevel']==1029 && empty($listingInfoForPercentage['WorkExperience']['WE_JobTitle'][1])) 
		{
		$percentage += 30;
		}
        return $percentage;
