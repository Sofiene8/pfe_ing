...
class SJB_Listing extends SJB_Object
{
	...
	function __construct($listingInfo = array(), $listing_type_sid = 0, $pageID = 0)
	{
		...
		$this->date_add = null;
        if (isset($listingInfo['activation_date'])) {
            $date = date_create($listingInfo['activation_date']);
            if ($date) {
                $this->activation_date = date_format($date, 'Y-m-d H:i:s');
            }
        }
		if (isset($listingInfo['update_date'])) {
            $date = date_create($listingInfo['update_date']);
            if ($date) {
                $this->update_date = date_format($date, 'Y-m-d H:i:s');
            }
        }
		if (isset($listingInfo['date_add'])) {
            $date = date_create($listingInfo['date_add']);
            if ($date) {
                $this->date_add = date_format($date, 'Y-m-d H:i:s');
            }
        }
	...
	function isActive()
	{
		return $this->active == SJB_Listing::STATUS_ACTIVE;
	}
    ...
    function addActivationDateProperty($activation_date = null)	{ return $this->details->addActivationDateProperty($activation_date); }
	 function addUpdateDateProperty($update_date = null)	{ return $this->details->addUpdateDateProperty($update_date); }
