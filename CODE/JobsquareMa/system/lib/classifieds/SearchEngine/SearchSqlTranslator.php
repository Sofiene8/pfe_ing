...
//            echo '<pre>';
//            var_dump($criteria['system']['listing_type'][0]->value);
//            var_dump($where_block);
//            echo '</pre>';
			if(isset($criteria['system']['listing_type']))
{
            if ($criteria['system']['listing_type'][0]->value == 7) {
               // $where_block .= " AND `listings`.`Photo` <> '' AND `listings`.`Phone` <> '' ";
			   $where_block .= "  AND `listings`.`Phone` <> '' ";
            }
			}
//            var_dump($where_block);
//        }
		...
		if(isset($criteria['user'])&& count($criteria['user'])>=1)
		{
		
		
		$from_block.=" inner join `users`  on `users`.sid=`listings`.user_sid ";
		}
		...
		//echo $select_block . $from_block . $where_block . $group_block . $having_block . $sorting_block . $limit_block;
...
//                var_dump($sorting_order);
                if (!preg_match('/asc|desc|rand/ui', $sorting_order)) {
//                    var_dump('ddd');
                    continue;
                }

//                if ($sorting_order == 'RAND()') {
//                    $sorting_block .= $delimiter .  " " . $sorting_order;
//                    $delimiter = ", ";
//                } else {
...
//                }
    ...
    function _getWhereStatement($criteria)
    {
		...
		if(isset($criteria['user'])&& count($criteria['user'])>=1)
		{
		return $this->_getWhereSystemStatement($criteria['system']) . $this->_getWhereCommonStatement($criteria['common']). $this->_getWhereUserStatement($criteria['user']);
		}
		else{
 ...
 function _getWhereUserStatement($criteria)
    {
		
        $where_user_block = '';
        foreach ($criteria as $property_criteria) {
            $where_AND_USER_block = '';
            foreach ($property_criteria as $criterion) {
			if($criterion->isValid() && SJB_UserProfileFieldManager::getUserProfileFieldInfoByID($criterion->property_name)) 
				{
                    ...
                    if ($criterion instanceof SJB_NullCriterion ||
                        ...
                        $where_AND_USER_block .= 'AND ' . $criterion->getUserSQL('users') . ' ';
						

						
                    } else {
                        if ($criterion instanceof SJB_CompanyLikeCriterion) {
                            $sql = $criterion->getUserSQL();
                            if ($sql !== null) {
                                $where_AND_USER_block .= 'AND ' . $sql . ' ';
                            }
                        } else {
                            $sql = $criterion->getUserSQL();
                            if ($sql !== null) {
                                $where_AND_USER_block .= "AND `users`." . $sql . ' ';
                            }
                        }
                    }
                }
            }
			 if (!empty($where_AND_USER_block))
                $where_user_block .= $where_AND_USER_block;
        }
	/*if(!empty($where_user_block)&&$where_user_block!='')
		{
		
		array_merge($inner_join, ['users' => ['join_field' => 'user_sid', 'join_field2' => 'sid', 'join' => 'INNER JOIN']]);
		//$inner_join = ['users' => ['join_field' => 'user_sid', 'join_field2' => 'sid', 'join' => 'INNER JOIN']];
		//$from_block.=" inner join `users`  on `users`.sid=`listings`.user_sid ";
		echo $inner_join;
		}*/
		
        return $where_user_block;
    }
