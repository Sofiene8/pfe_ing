...
class SJB_SearchCriterion
{
	...
	public function __construct($criterion_type)
	{
	...
	public static function getCriterionByType($type)
	{
		$types = [
			...
			'user_like' => 'SJB_UserLikeCriterion',
...
class SJB_LikeCriterion extends SJB_SearchCriterion
{
	function getSQL( $table_name = '' )
	{
		...
		if($table_name)
			...
			return "(`{$table_name}`.`id` = '".$id . "'  AND MATCH (`{$table_name}`.`value`) AGAINST ('{$value}'))";
			else*/
			return "(`{$table_name}`.`id` = '".$id . "' AND `{$table_name}`.`value` LIKE '%{$value}%')";
			}
		else
		{
			...
			/*if($table_name=='listings')
				return "(`id` = '".$id . "'  MATCH(`value`) AGAINST ('{$value}') )";
				else*/
				return "(`id` = '".$id . "' AND `value` LIKE '%{$value}%')";
...
class SJB_UserLikeCriterion extends SJB_SearchCriterion
{
	function getSQL( $table_name = '' )
	{
	...
	function getSystemSQL($table = '')
	{
		...
		$value = SJB_DB::quote($this->value);
		$value = str_replace($search, $replace, $value);
		return "({$tablePrefix}`{$this->property_name}` LIKE '%{$value}%' OR {$tablePrefix}`CompanyName` LIKE '%{$value}%' OR {$tablePrefix}`FullName` LIKE '%{$value}%')";
	}
...
class SJB_InCriterion extends SJB_SearchCriterion
{
	...
	function getSQLValue()
	{
		$value 		= '';
		if (is_array($this->value))
			$value = implode(', ',$this->_wrapArrayWithApostrof($this->value));
	...
	function getSQL()
	{
		...
		if (is_array($values)) {
			foreach ($values as $value) {
				...
				if ($res == '') {
					$res .= "`value` like '%{$val}%'";
					//$res .= " MATCH (`value`) AGAINST ('{$val}')";
				} else {
					$res .= " AND `value` like '%{$val}%'";
				//	$res .= "AND MATCH (`value`) AGAINST ('{$val}')";
	...
	function getSystemSQL($table = '')
	{
		...
		foreach ($values as $val) {
			...
			/*if($tab == 'listings')
				...
				$res .= " MATCH ({$table}`{$id}`) AGAINST ('{$val}')";
				else
				$res .= "{$table}`{$id}` like '%{$val}%'";
			*/
			}
				...
				/*if($tab == 'listings')
				$res .= " AND MATCH ({$table}`{$id}`) AGAINST ('{$val}')";
				else
				$res .= " AND {$table}`{$id}` like '%{$val}%'";
				*/
			}
