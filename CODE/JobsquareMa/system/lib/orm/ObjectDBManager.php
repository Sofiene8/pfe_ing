...
class SJB_ObjectDBManager
{
    ...
    /**
     * Batch fetch multiple objects by SIDs in 2 queries (main table + properties table).
     * Returns associative array keyed by SID.
     */
    public static function getObjectsInfoBySIDs($dbTableName, array $sids)
    {
        if (empty($sids)) {
            return [];
        }

        $rows = SJB_DB::query("SELECT * FROM ?w WHERE sid IN (?l)", $dbTableName, $sids);
        $objectsByKey = [];
        foreach ($rows as $row) {
            $objectsByKey[$row['sid']] = $row;
        }

        // Process location fields and add_parameter for each object
        foreach ($objectsByKey as $sid => &$objectInfo) {
            foreach ($objectInfo as $key => $val) {
                $locationFields = ["{$key}_Country", "{$key}_State", "{$key}_City", "{$key}_ZipCode", "{$key}_Latitude", "{$key}_Longitude"];
                if (array_key_exists("{$key}_Country", $objectInfo) && array_key_exists("{$key}_State", $objectInfo) && array_key_exists("{$key}_City", $objectInfo)) {
                    $objectInfo[$key] = [];
            ...
            if ($dbTableName == 'listings' && !empty($objectInfo['complex'])) {
                $complexFields = unserialize($objectInfo['complex']);
                if ($complexFields) {
                    $objectInfo = array_merge($complexFields, $objectInfo);
                }
        ...
        if (SJB_DB::table_exists($dbTableName . "_properties")) {
            $allProps = SJB_DB::query("SELECT * FROM ?w WHERE object_sid IN (?l)", $dbTableName . "_properties", $sids);
            foreach ($allProps as $prop) {
                $objSid = $prop['object_sid'];
                if (!isset($objectsByKey[$objSid])) {
                    continue;
                }
                if (isset($prop['add_parameter']) && $prop['add_parameter'] != '') {
                    $objectsByKey[$objSid][$prop['id']]['add_parameter'] = $prop['add_parameter'];
                    $objectsByKey[$objSid][$prop['id']]['value'] = $prop['value'];
                } else {
                    $objectsByKey[$objSid][$prop['id']] = $prop['value'];
                }
            }
        }
