...
class SJB_UploadFileManager
{
 ...
 function setFileTime($file_time)
    {
    ...
    function isValidUploadedFile($fileId, $withFormats = true)
    {
        ...
        if ($this->error === false && $withFormats) {
            $fileFormats = explode(',', SJB_System::getSettingByName('file_valid_types'));
            $fileInfo = pathinfo($_FILES[$fileId]['name']);

            if (empty($fileInfo['extension']) && class_exists('finfo')) {
                $finfo = new finfo(FILEINFO_MIME_TYPE);
                $mime = $finfo->file($_FILES[$fileId]['tmp_name']);
                $mimeToExtension = [
                    'application/pdf' => 'pdf',
                    'application/vnd.openxmlformats-officedocument.wordprocessingml.document' => 'docx',
                ];
                // добавим экстеншн если его нет
                if (isset($mimeToExtension[$mime])) {
                    $fileInfo['extension'] = $mimeToExtension[$mime];
                    $_FILES[$fileId]['name'] .= '.' . $mimeToExtension[$mime];
                }
            }
    ...
    public static function isFileReadyForUpload($file_id)
    {
 ...
 function uploadFileApplication($file_id,$listing_id)
    {
        if (is_null($this->uploaded_file_id)) {
            return false;
        }
        if (!empty($_FILES[$file_id]['name'])) {
			...
			$time=time();
			$this->setFileTime($time);
            list($file_name, $saved_file_name) = self::getArrayOfFileApplicationsNames($_FILES[$file_id]['name'],$listing_id);
    ...
    function getArrayOfFileNames($fileBasename)
    {
		...
		if($this->file_time)
			{
			$dir_date=SJB_UploadFileManager::getdatedirectory($uploadFileDirectory ."/". $this->file_group,$this->file_time);
            $fileName = $dir_date. $savedFileName;
			}
			else
			{
...
function getArrayOfFileApplicationsNames($fileBasename,$listing_id)
    {
        $uploadFileDirectory = "files/applications";
        $fileExtension = strrchr($fileBasename, ".");
        if (!empty($fileExtension)) {
            $fileNameWithoutExt = substr($fileBasename, 0, -strlen($fileExtension));
        } else {
		...
		if($this->file_time)
			{
			$dir_date=SJB_UploadFileManager::getapplicationdatedirectory($uploadFileDirectory,$this->file_time,$listing_id);
            $fileName = $dir_date. $savedFileName;
			}
			else
			{
			$fileName = $uploadFileDirectory . "/" .$listing_id. "/" . $savedFileName;
			 }
    ...
    public static function deleteUploadedFile($file_info, $file_id)
    {
		...
		if($file_info['creation_time'])
		{
		
		$path=self::getdirectoryFromTime($upload_file_directory."/".$file_info['file_group'],$file_info['creation_time']);
		
        $file_name = SJB_Path::combine($path, $file_info['saved_file_name']);
		}
		else
		{
    ...
    public static function getUploadedFileLink($uploaded_file_id, $file_info = false, $noHost = false)
    {
        ...
        if (!empty($file_info)) {
			...
			if($file_info['creation_time'])
			{
			$dir_date=SJB_UploadFileManager::getdirectoryFromTime($upload_files_directory."/".$file_group,$file_info['creation_time']);
            $file_name = $dir_date."/".$saved_file_name;
			}
			else
			{
	...
	public static function getUploadedFileApplicationLink($uploaded_file_id, $file_info, $noHost,$listing_id)
    {
        if ($file_info === false) {
            $file_info = SJB_UploadFileManager::getUploadedFileInfo($uploaded_file_id);
        }
        if (!empty($file_info)) {
            $upload_files_directory = "files/applications";
            $file_group = $file_info['file_group'];
            $saved_file_name = $file_info['saved_file_name'];
			
			$dir_date=SJB_UploadFileManager::getapplicationdirectoryFromTime($upload_files_directory,$file_info['creation_time'],$listing_id);
            $file_name = $dir_date."/".$saved_file_name;
    ...
    public static function getUploadedFileSize($uploaded_file_id)
    {
		...
		if($file_info['creation_time'])
			{
			$dir_date=SJB_UploadFileManager::getdirectoryFromTime($uploadFileDirectory ."/". $fileInfo['file_group'],$file_info['creation_time']);
            $file_name = $dir_date. $fileInfo['saved_file_name'];
			}
			else
 ...
 public static function openApplicationFile2($filename, $appsID)
    {
        $file_info = SJB_UploadFileManager::getMimeTypeAppsByFilename($filename, $appsID);
		 $apps= SJB_DB::query("SELECT `applications`.`listing_id` from `applications`   WHERE `applications`.`id` = ?s ", $appsID);
      $app=$apps? array_pop($apps) : false;
	  if($app)
	  $listing_id=$app['listing_id'];
        if ($file_info) {
            $file_link = SJB_UploadFileManager::getUploadedFileApplicationLink($file_info['id'], false, true,$listing_id);
    ...
    public function copyFile($uploadedFileInfo, $fileID)
    {
        ...
        $uploadFileDirectory = SJB_System::getSystemSettings('UPLOAD_FILES_DIRECTORY');
		if($uploadedFileInfo['creation_time'])
			{
			$dir_date=SJB_UploadFileManager::getdirectoryFromTime($uploadFileDirectory ."/". $uploadedFileInfo['file_group'],$uploadedFileInfo['creation_time']);
            $dir_name = $dir_date."/". $savedFileName;
			 if (copy($dir_name. '/' . $uploadedFileInfo['saved_file_name'], $fileName)) {
    ...
    public static function deleteUploadedFilesByListingSID($listingSid)
    {
        ...
        foreach ($listing->getProperties() as $property) {
            if (in_array($property->getType(), ['file', 'picture'])) {
                $uploadedFileId = null;
	...
	public static function getdatedirectory($directory,$time){
...
$date->setTimestamp($time);
	...
	$dir_an=$directory."/".$date2[0];
	$dir_month=$directory."/".$date2[0]."/".$date2[1];
	$dir_day=$directory."/".$date2[0]."/".$date2[1]."/".$date2[2]."/";
	if (!is_dir($dir_an) ){
    mkdir($dir_an, 0777, true);
		}
	if (!is_dir($dir_month) ){
    mkdir($dir_month, 0777, true);
		}
		if (!is_dir($dir_day) ){
    mkdir($dir_day, 0777, true);
		}
		return $dir_day;
	}
	
	public static function getapplicationdatedirectory($directory,$time,$listing_id){
	
	 $jobInfo = SJB_ListingManager::getListingInfoBySID($listing_id);

	//$datetimeFormat = 'Y-m-d H:i:s';
	//$date = new \DateTime();
	$date_add= $jobInfo['date_add'];
	
// If you must have use time zones
// $date = new \DateTime('now', new \DateTimeZone('Europe/Helsinki'));
	//$date->setTimestamp($date_add);
	//$date1=$date->format($datetimeFormat);
	
	$date1=explode(" ",$date_add);
	$date2=explode("-",$date1[0]);
	$dir_an=$directory."/".$date2[0];
	$dir_month=$directory."/".$date2[0]."/".$date2[1];
	$dir_day=$directory."/".$date2[0]."/".$date2[1]."/".$date2[2];
	$dir_listing=$directory."/".$date2[0]."/".$date2[1]."/".$date2[2]."/".$listing_id."/";
	if (!is_dir($dir_an) ){
    mkdir($dir_an, 0777, true);
		}
	if (!is_dir($dir_month) ){
    ...
    mkdir($dir_day, 0777, true);
		...
		if (!is_dir($dir_listing) ){
    mkdir($dir_listing, 0777, true);
		}
		
		return $dir_listing;
	}
public static function getdirectoryFromTime($directory,$time){
...
$date->setTimestamp($time);
$date1=$date->format($datetimeFormat);;
$datetoday="2021-02-24";
	$date1=explode(" ",$date1);
	if(trim($date1[0])>=$datetoday)
	{
	$date2=explode("-",$date1[0]);
	//$dir_an=$directory."/".$date2[0];
	//$dir_month=$directory."/".$date2[0]."/".$date2[1];
	$dir=$directory."/".$date2[0]."/".$date2[1]."/".$date2[2];
	}
	else
	{
	...
	public static function getapplicationdirectoryFromTime($directory,$time,$listing_id){
	$jobInfo = SJB_ListingManager::getListingInfoBySID($listing_id);
	$date_add=$jobInfo['date_add'];
	$datetimeFormat = 'Y-m-d H:i:s';
	$date = new \DateTime();
	...
	$date->setTimestamp($time);
	$date1=$date->format($datetimeFormat);
	$datetoday="2021-04-15";
	$datetoday2="2021-02-24";
	$dir_files="files/files";
	$date1=explode(" ",$date1);
	
	if( trim($date1[0])>=$datetoday2 && trim($date1[0])<$datetoday)
	{

	$date2=explode("-",$date1[0]);
	//$dir_an=$directory."/".$date2[0];
	//$dir_month=$directory."/".$date2[0]."/".$date2[1];
	$dir=$dir_files."/".$date2[0]."/".$date2[1]."/".$date2[2];
	}
	elseif(trim($date1[0])>=$datetoday)
	{
	
	//$date_app->setTimestamp($date_add);
//	$date_app=$date_app->format($datetimeFormat);
	$date3=explode(" ",$date_add);
	$date_ap=explode("-",$date3[0]);
	//$dir_an=$directory."/".$date2[0];
	//$dir_month=$directory."/".$date2[0]."/".$date2[1];
	$dir=$directory."/".$date_ap[0]."/".$date_ap[1]."/".$date_ap[2]."/".$listing_id;
	
	//$dir=$directory."/".$listing_id;
