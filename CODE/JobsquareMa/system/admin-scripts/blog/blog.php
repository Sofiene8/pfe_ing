...
class SJB_Admin_Blog_Blog extends SJB_Function
{
    ...
    {
        $this->setPermissionLabel(SJB_Acl::ADMIN_CONTENT);
    ...
    public function execute()
    {
        ...
        switch ($action) {
                ...
                if (!$postId) {
                    ...
                    if ($formSubmitted) {
                        ...
                        if ($url) {
                            ...
                            $_REQUEST['url'] = sprintf('/%s/%s/', $postId, SJB_TemplateProcessor::pretty_url(SJB_Request::getVar('title')));
                        }
                    }
                    ...
                    if ($formSubmitted) {
                        if (SJB_BlogManager::isUrlExists($blogPost->getPropertyValue('url'), $postId)) {
                            $errors[] = 'Blog post with such url already exists';
                        }
                    }
                    if ($formSubmitted && $blogPostForm->isDataValid($errors)) {
                        $blogPost->setSID($postId);
                        SJB_BlogManager::saveBlogPost($blogPost);
                    }
                }
                $formFields = $blogPostForm->getFormFieldsInfo();
                $tp->assign('form_fields', $formFields);
                $tp->assign('post_info', $postInfo);
                $tp->assign('errors', $errors);
                $tp->assign('blog_categories', SJB_BlogManager::getCategories());
            ...
            case 'add':
                ...
                $formSubmitted = SJB_Request::getVar('form_submit', false);
                if ($formSubmitted) {
                    if (SJB_BlogManager::isUrlExists($blogPost->getPropertyValue('url'))) {
                        $errors[] = 'Blog post with such url already exists';
                    }
                }
                if ($formSubmitted && $blogPostForm->isDataValid($errors)) {
                    SJB_BlogManager::saveBlogPost($blogPost);
                    if ($blogPost->getSID()) {
                        if (!trim($blogPost->getPropertyValue('url'))) {
                            $blogPost->setPropertyValue('url', sprintf('/%s/%s/', $blogPost->getSID(), SJB_TemplateProcessor::pretty_url($blogPost->getPropertyValue('title'))));
