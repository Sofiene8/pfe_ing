...
<ul class="list-unstyled browse-by__list">
	{foreach from=$browseItems item="item"}
		{if $item.count > 0 && $i <= $recordsNumToDisplay}
			...
			<li>
				<a href="{$GLOBALS.site_url}/cities/jobs-in-{$item.caption|escape:htmlall:'UTF-8'|regex_replace:"/[-\/]/":""}/">
					<span class="browse-by__item">Emploi à [[{$item.caption|escape|truncate:28:"...":true}]]</span>
