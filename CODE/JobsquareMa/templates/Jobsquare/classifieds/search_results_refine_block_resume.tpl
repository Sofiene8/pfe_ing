...
{if !empty($refineFields)}
				...
				<div class="collapse in clearfix" id="refine-block-{$refineField.field_name}">
					{foreach from=$refineField.search_result item=val name=fieldValue}
						...
						<a class="refine-search__item" href="?{$smarty.capture.urlParams}&amp;{$smarty.capture.refineFieldCriteria}">
							...
							{if $val.count != 0}<span class="refine-search__count">{if empty($refineField.criteria)}&nbsp;({$val.count}){/if}</span>{/if}
						</a>
					{/foreach}
	...
	<!--
	<div class="refine-search__block">
			<a class="btn__refine-search" role="button" data-toggle="collapse" href="#refine-block-{$refineField.field_name}" aria-expanded="true" aria-controls="refine-block-{$refineField.field_name}">Photo</a>
			<div class="collapse in clearfix" id="refine-block-{$refineField.field_name}">
				<a class="refine-search__item" href="?{$smarty.capture.urlParams}&amp;Photo[not_equal][]=">
					<span class="refine-search__value">{tr}Avec{/tr|escape}</span>
				</a>
				<a class="refine-search__item" href="?{$smarty.capture.urlParams}&amp;Photo[equal][]=">
					<span class="refine-search__value">{tr}Sans{/tr|escape}</span>
				</a>
				<a class="refine-search__item" href="?searchId={$searchId|escape:'url'}&amp;action=undo&amp;param=Photo&amp;type=equal&amp;value=">
					<span class="refine-search__value">{tr}Tous{/tr|escape}</span>
				</a>
			</div>
		</div>
		-->
