...
.date-separator {
    display: none; /* Completely remove the separator since it's not needed */
        ...
        .modal {
            z-index: 1055 !important; /* Higher than default to avoid overlap */
        }
...
<div class="filters-container">
    <div class="filters-header">
		...
		<button class="btn btn-secondary btn-export-pdf" onclick="exportFilteredPDF()">
            ...
            <button class="btn btn-secondary btn-export-list" onclick="exportFilteredData()">
...
function resetAllFilters() {
    // VERSION CORRIGÉE : On ne réinitialise que les champs qui existent.
    ...
    // On récupère les valeurs de tous les filtres, comme dans applyFilters
    const filters = {
        ...
        // 'contractType' a été retiré du HTML, donc on le retire ici aussi.
    ...
    // On construit une chaîne de paramètres URL
    const params = new URLSearchParams();
    params.append('export_action', 'csv'); // L'action qui déclenche l'export en PHP
    params.append('appJobId', appJobId);

    for (const key in filters) {
        if (filters[key]) { // On ajoute seulement les filtres qui ont une valeur
            params.append(key, filters[key]);
        }
    }

    // On redirige le navigateur vers l'URL d'export, ce qui déclenchera le téléchargement
...
function exportFilteredPDF() {
    ...
    const params = new URLSearchParams();
    params.append('export_action', 'pdf'); // new action
    params.append('appJobId', appJobId);
...
function applyFilters() {
    // VERSION CORRIGÉE : On ne lit que les valeurs des champs qui existent réellement dans le HTML.
    const filters = {
        search: document.getElementById('search-filter').value.trim(),
        status: document.getElementById('status-filter').value,
        date_month: document.getElementById('date-month-filter').value,
...
if (yearSelect) {
  ...
  // simplest: set the value directly
    ...
    for (var i = 0; i < yearSelect.options.length; i++) {
      if (yearSelect.options[i].value === currentYear) {
        ...
        function initializeEventHandlers() {
            $('.applicant-card, .application-item').off('click').on('click', function(e) {
            ...
            $('.application-details__contact').off('click').on('click', function(e) {
                e.preventDefault();
            ...
            $('.update-notes').off('click').on('click', function() {
            ...
            $('.application-details__modal select').off('change').on('change', function() {
        ...
        function sortApplications(sortBy) {
            const container = document.querySelector('.applicants-list-view.active');
            if (!container) return;
            const cards = Array.from(container.querySelectorAll('.application-item'));
            cards.sort((a, b) => {
                const aData = {
                    name: a.querySelector('.application-name')?.textContent.toLowerCase() || '',
                    date: new Date(a.querySelector('.application-date')?.textContent || 0),
                    experience: parseInt(a.dataset.experience || '0')
                };
                const bData = {
                    name: b.querySelector('.application-name')?.textContent.toLowerCase() || '',
                    date: new Date(b.querySelector('.application-date')?.textContent || 0),
                    experience: parseInt(b.dataset.experience || '0')
                };
                switch (sortBy) {
                    case 'date_asc': return aData.date - bData.date;
                    case 'date_desc': return bData.date - aData.date;
                    case 'name_asc': return aData.name.localeCompare(bData.name);
                    case 'name_desc': return bData.name.localeCompare(bData.name);
                    case 'experience_asc': return aData.experience - bData.experience;
                    case 'experience_desc': return bData.experience - aData.experience;
                    default: return 0;
                }
            });
                                                ...
                                                <div class="details-header__right">
                                                    ...
                                                    <ul class="listing-item__info clearfix">
                                                        ...
                                                        {if $application.resumeInfo && $application.resumeInfo.OtherPhone}
                                                            <li class="listing-item__info--item listing-item__info--item-phone">
                                                                <a href="tel:{$application.resumeInfo.OtherPhone}">{$application.resumeInfo.OtherPhone}</a>
                                                            </li>
                                                        {/if}
                                                    </ul>
                                                    <ul class="listing-item__info clearfix">
