...
<div class="link-app" id="applicants-container-links">
{foreach from=$statuses key="status" item=count}
<a href="#{$status}" class="btn-next-{$status}">{$status}</a>
...
<div class="applicants" id="applicants-container">
    ...
    {foreach from=$statuses key="status" item=count}
        ...
        <div class="applicant-status" data-status="{$status|escape}" id="{$status|escape}">
...
$(document).ready(function () {
  ...
  $('.btn-next').on('click', function (e) {
    ...
    // section actuelle
    const $currentSection = $(this).next('.applicant-status');

    // section suivante
    const $nextSection = $currentSection.next('.applicant-status');
...
$(document).ready(function () {
  ...
  $('#applicants-container-links a').on('click', function (e) {
    e.preventDefault();

    const targetId = $(this).attr('href');
    const $target = $(targetId);
    const $container = $('.applicants');

    if ($target.length) {
      const scrollLeft =
        $target.position().left + $container.scrollLeft();
